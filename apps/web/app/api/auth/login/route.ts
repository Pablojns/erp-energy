...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ message: 'Não foi possível contatar o servidor da API.' }, { status: 502 });
  }
  ...
  if (!upstream.ok) {
    ...
    return NextResponse.json({ message }, { status: upstream.status >= 400 ? upstream.status : 502 });
  }
  ...
  if (!accessToken) {
    return NextResponse.json({ message: 'Resposta inválida da API (sem accessToken).' }, { status: 502 });
  }
