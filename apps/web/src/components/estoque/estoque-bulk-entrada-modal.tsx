...
export function EstoqueBulkEntradaModal(props: {
  ...
  useEffect(() => {
    ...
    // Só recarrega ao abrir / mudar contexto — callbacks do pai são estáveis o suficiente.
  ...
  const confirm = async () => {
    ...
    const payload: Array<{ productId: string; quantity: number; name: string }> =
      [];
          ...
          <div className="flex shrink-0 items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-1 text-xs text-[var(--text-secondary)]">
                Busque, selecione produtos e informe a quantidade de cada um.
              </p>
            </div>
            <button
              type="button"
                      ...
                      >
                        ...
                        <td className="px-2 py-2 text-right">
                          <input
                            ...
                            placeholder="0"
                            className="w-24 rounded-lg border border-[var(--border-color)] bg-[var(--bg-card)] px-2 py-1.5 text-right text-sm tabular-nums outline-none disabled:opacity-50"
                          />
                        </td>
                      </tr>
                    );
