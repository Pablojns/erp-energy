...
## ERP-ENERGY context (project defaults)

When reviewing this monorepo, include these areas unless the diff is unrelated:

- **Auth**: JWT, `AUTH_DISABLED` bypass in dev, tokenVersion invalidation, role/permission checks
- **Data exposure**: customer/order/financial data in logs, API responses, chat/notifications
- **Integrations**: Correios credentials, R2/S3 uploads, external order imports (WEG, e-commerce)
- **Input validation**: DTOs, file uploads (NF, etiquetas), SQL injection via Prisma raw queries
- **Secrets**: never commit `.env`; check for hardcoded keys in controllers/services

Default `Custom Instructions` for this repo unless the user overrides:

```text
Monorepo ERP-ENERGY. Prioritize:
1) authentication/authorization bypass (JwtGuard, PermissionGuard, public endpoints)
2) IDOR on pedidos, financeiro, CRM cards, purchase requests
3) mass assignment via DTOs missing whitelist/validation
4) file upload abuse (NF, images) and presigned URL leaks
5) sensitive data in logs (Pino), audit trail gaps, and missing rate limits on auth/import endpoints
```
