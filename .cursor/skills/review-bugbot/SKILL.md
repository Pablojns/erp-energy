...
- **API (`apps/api`)**: NestJS controllers/services, JWT auth, permission guards, DTO validation, Prisma transactions
- **Web (`apps/web`)**: Next.js pages/components, API client calls, permission-aware UI
- **Database (`packages/database`)**: Prisma schema migrations, enum/status transitions, stock reservation flows
- **Critical domains**: pedidos/expedição, estoque/reservas, financeiro, CRM, compras, Correios/R2 uploads

Default `Custom Instructions` for this repo unless the user overrides:

```text
Monorepo ERP-ENERGY (NestJS + Next.js + Prisma/PostgreSQL). Prioritize:
1) regressions in order status transitions and stock movements (RESERVA, SAIDA_EXPEDICAO)
2) permission bypasses (@RequirePermission, JwtGuard)
3) missing DTO validation and unsafe Prisma queries (N+1, missing transactions)
4) breaking changes between apps/api and apps/web contracts
5) cron jobs (financeiro, crm, notifications) with idempotency and error handling
```
